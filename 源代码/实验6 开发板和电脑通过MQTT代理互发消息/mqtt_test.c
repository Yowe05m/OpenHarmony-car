...
int mqtt_connect(void)
{
	...
	NetworkInit(&n);
	NetworkConnect(&n, "192.168.114.144", 1883);
	...
	data.password.cstring = "md205525";
	data.cleansession = 1;
	...
	while(1)
	{
		...
		message.qos = QOS1;
		message.retained = 0;
		const char student_id[] = "25120232201399";
		message.payload = (void *)student_id;
		message.payloadlen = strlen(student_id);
